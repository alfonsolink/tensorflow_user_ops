...
def _roi_warping_grad(op, grad, h, w):
  ...
  # compute gradient
  data_grad, buffer_ = roi_warping_op.roi_warping_grad(data, rois, argmax_h, argmax_w, grad, pooled_height, pooled_width, spatial_scale)

  return [None, data_grad]  # List of one Tensor, since we have one input
