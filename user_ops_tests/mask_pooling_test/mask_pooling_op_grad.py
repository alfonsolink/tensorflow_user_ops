...
def _mask_pool_grad(op, grad):
  ...
  # compute gradient
  data_grad, mask_grad = mask_pooling_op.mask_pooling_grad(data, masks, grad, height, width)

  return [data_grad, mask_grad]  # List of one Tensor, since we have one input
