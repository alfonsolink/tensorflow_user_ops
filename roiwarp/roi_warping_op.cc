...
static void RoiWarpingGradKernel(
    ...
    const int height, const int width, const int pooled_height,
    const int pooled_width, const TensorShape& tensor_output_shape, const TensorShape& buffer_output_shape, const TensorShape& tensor_feature_output_shape)  
{
  Tensor* output = nullptr;
  Tensor* buffer_data = nullptr;
  Tensor* feature_output = nullptr;
  OP_REQUIRES_OK(context, context->allocate_output(0, tensor_output_shape, &output));
  OP_REQUIRES_OK(context, context->allocate_output(1, buffer_output_shape, &buffer_data));
  OP_REQUIRES_OK(context, context->allocate_output(2, tensor_feature_output_shape, &feature_output));
  ...
  ROIWarpBackwardLauncher(
    ...
    output->flat<float>().data(), buffer_data->flat<float>().data(), feature_output->flat<float>().data(), argmax_data_h->flat<int>().data(), 
...
class RoiWarpGradOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override 
  {
    ...
    TensorShape feature_output_shape = bottom_data.shape();
    ...
    RoiWarpingGradKernel(
      ...
      pooled_width_, output_shape, buffer_shape, feature_output_shape);
