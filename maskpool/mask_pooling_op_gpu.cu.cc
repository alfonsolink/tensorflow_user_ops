                              ...
                              const int width, const Eigen::GpuDevice& d) 
{
      ...
      SetZero<<<(output_size + kThreadsPerBlock - 1) / kThreadsPerBlock, kThreadsPerBlock, 0, d.stream()>>>(output_size, bottom_mask_diff);
      
      MaskPoolingBackwardMask<<<(output_size + kThreadsPerBlock - 1) / kThreadsPerBlock, kThreadsPerBlock, 0, d.stream()>>>
      (output_size, bottom_data, bottom_mask_diff, top_diff, channels, height, width);
