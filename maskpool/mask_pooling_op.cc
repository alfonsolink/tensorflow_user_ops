...
static void MaskPoolingGradKernel(
    ...
    const int num_masks, const int channels, const int height, const int width, const TensorShape& tensor_output_shape, const TensorShape& mask_output_shape) 
{
  Tensor* output = nullptr;
  Tensor* output_masks = nullptr;
  OP_REQUIRES_OK(context, context->allocate_output(0, tensor_output_shape, &output));
  OP_REQUIRES_OK(context, context->allocate_output(1, mask_output_shape, &output_masks));
  ...
  MaskPoolBackwardLauncher(
    bottom_data->flat<float>().data(), bottom_masks->flat<float>().data(), output->flat<float>().data(), output_masks->flat<float>().data(),
...
class MaskPoolGradOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override 
  {
    ...
    TensorShape mask_output_shape = bottom_masks.shape();

    MaskPoolingGradKernel(
      context, &bottom_data, &bottom_masks, &out_backprop,
      num_masks, channels, height, width, output_shape, mask_output_shape);
